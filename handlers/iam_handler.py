...
def scan_unused_iam(region, session=None):
...
def delete_unused_iam(issue, region, session=None):
    session = session or boto3.Session()
    iam = session.client('iam')
    iam.delete_user(UserName=issue['user'])

def analyze_iam_policies(region, session=None):
    session = session or boto3.Session()
    iam = session.client('iam')
...
def fix_iam_policy(issue, region, session=None):
    session = session or boto3.Session()
    iam = session.client('iam')
